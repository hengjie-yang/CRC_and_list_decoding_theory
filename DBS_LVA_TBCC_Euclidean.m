function [check_flag,correct_flag, path_rank, dec]=DBS_LVA_TBCC_Euclidean(trellis, rxSig, input, L)
%
% Input: 
%       1)trellis: the extended ZTCC trellis; 
...
NumOutBit=log2(trellis.numOutputSymbols);
NumState=trellis.numStates;
v=log2(NumState);
% length=size(rxSig,2)/NumOutBit;%message length (aborted)
length = v + size(rxSig,2)/NumOutBit + v; % changed to extended trellis length
...
for iteration=0:length-1
    ...
    if iteration >=v && iteration < length - v
        received_point=rxSig(NumOutBit*(iteration-v)+1: NumOutBit*(iteration-v)+NumOutBit);
    end
    
    %update next_state
    if iteration == 0
        for jj=1:NumInput
            next_state=trellis.nextStates(1,jj)+1;
            if isempty(Column{iteration+1}{next_state})
                Column{iteration+1}{next_state}=cell(2,1); 
                Column{iteration+1}{next_state}{1}=[0, 1]; % [minimum_metric, father state] by default, the optimal metric is zero
                Column{iteration+1}{next_state}{2}=[Inf, -1];%[difference, state]
            end
        end
    elseif iteration > 0 && iteration < v
        for ii = 1: NumState
            if ~isempty(Column{iteration}{ii})
                for jj=1:NumInput
                    next_state=trellis.nextStates(ii,jj)+1;
                    if isempty(Column{iteration+1}{next_state})
                        Column{iteration+1}{next_state}=cell(2,1);%minimum_metric, father state
                        Column{iteration+1}{next_state}{1}=[0, ii]; % by default, the optimal metric is zero
                        Column{iteration+1}{next_state}{2}=[Inf, -1];%[difference, state]
                    end
                end
            end
        end
    elseif iteration >=v && iteration < length-v %message part
        for i=1:NumState
            if ~isempty(Column{iteration}{i})
                for j=1:NumInput
                    ...
                    if isempty(Column{iteration+1}{next_state})
                        ...
                        Column{iteration+1}{next_state}{2}=[Inf, -1];
                    elseif Column{iteration+1}{next_state}{1}(1)>diff_metric
    ...
    else % iteration >= length - v
        for i=1:NumState
            if ~isempty(Column{iteration}{i})
                next_state=trellis.nextStates(i,1)+1;
                diff_metric = Column{iteration}{i}{1}(1);
                if isempty(Column{iteration+1}{next_state})
                    Column{iteration+1}{next_state}=cell(2,1);%minimum_metric, father state
                    Column{iteration+1}{next_state}{1}=[diff_metric,i];
                    Column{iteration+1}{next_state}{2}=[Inf,-1];
...
%     end
    ...
    % TB condition check
    start_state_vec = codeword(1:v);
    end_state_vec = codeword(end-2*v+1: end-v);
    if all(start_state_vec == end_state_vec)
        check_flag = 1;
        path_rank = counter;
        dec = codeword;
        if all(codeword == input)
...
for iteration=length:-1:1
    ...
    for i=1:NumInput
        if trellis.nextStates(pre_state,i)+1==current_state
